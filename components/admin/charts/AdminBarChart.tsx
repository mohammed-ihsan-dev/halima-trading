...
export default function AdminBarChart({ data, type = "dual", height = 200 }: AdminBarChartProps) {
  ...
  return (
    <div
      ...
      className="w-full flex items-end justify-between gap-3 border-b border-slate-100 pb-3 px-2"
    >
      {data.map((item, index) => {
        if (type === "dual") {
          ...
          return (
            <div key={item.name} className="flex-1 flex flex-col items-center gap-2 h-full justify-end group">
              ...
              <span className="text-xs font-bold text-slate-700 truncate max-w-[80px]" title={item.name}>
                {item.name}
              </span>
            </div>
          );
        ...
        return (
          <div key={item.name} className="flex-1 flex flex-col items-center gap-2 h-full justify-end group">
            <span className="text-xs md:text-sm font-extrabold text-slate-700">{count}</span>
            ...
            <span className="text-xs md:text-sm font-bold text-slate-700 truncate mt-1 max-w-[100px]" title={item.name}>
              {item.name}
            </span>
          </div>
        );
